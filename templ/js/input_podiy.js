   ...
   jQuery('#login_cur_user').text() +"  </li>");
    ...
    var options = { // опции для отправки формы с помощью jquery form
        data: { // дополнительные параметры для отправки вместе с данными формы
            action: 'reg_podiy', // этот параметр будет указывать wp какой экшн запустить, у нас это wp_ajax_nopriv_add_object_ajax
            nonce: ajaxdata.reg_podiy_nonce // строка для проверки, что форма отправлена откуда надо
        },
    ...
    jQuery('#dod_declar').click(function(ff) { // по клику на ссылку "Добавить еще фото"
        ...
        var a1 = jQuery(this).parent().parent();
        a1.before("<li class='val_data' style='margin-left:22%;'> <input type='file' name='declar[]' /> </li>");
        //jQuery(this).before("<input type='file' name='declar[]' />");
});

 ...
 */
 ...
 <form name="loginform" id="loginform" method="post" class="userform" action=""> <!-- обычная форма, по сути нам важен только класс -->
 	<input type="text" name="log" id="user_login" placeholder="Логін" hidden> <!-- сюда будут писать логин или email -->
 	<input type="password" name="pwd" id="user_pass" placeholder="Пароль" hidden> <!-- ну пароль -->
 	<input name="rememberme" type="checkbox" value="forever" hidden=""> <!-- Запомнить меня запомнить ли сессию, forever - навсегда,  -->

<div style="margin-top:1em; margin-bottom:1em;" hidden>
...
<p style="margin:0px; margin-top: -50px;">Виберіть соц.мережу для входу:<?php echo ulogin_panel(); ?> </p>


  <input type="submit" value="Ввійти"  hidden> <!-- субмит -->
...
<form name="sms_form" id="sms_form" method="post" class="sms_userform" action="" hidden=""> <!-- обычная форма, по сути нам важен только класс -->
  	<input type="text" name="sms" id="sms" placeholder="Ввідіть код з СМС" hidden> <!-- сюда будут писать логин или email -->
  		<input type="submit" value="Підтвердити" disabled="" hidden> <!-- субмит -->

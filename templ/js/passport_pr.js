...
jQuery(document).ready(function() {
    ...
    jQuery(".change_prihil").click(function(ggg) { // по клику на ссылку "Добавить еще фото"
        ...
        jQuery.ajax({
            url: ajaxchange.url,
            data: { // дополнительные параметры для отправки вместе с данными формы
                action: 'change_passport_ajax', // этот параметр будет указывать wp какой экшн запустить, у нас это wp_ajax_nopriv_add_object_ajax
                nonce: ajaxchangepassport.nonce, // строка для проверки, что форма отправлена откуда надо
                id_p: a2.text(),
            },
            dataType: 'json',
